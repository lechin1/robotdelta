...
public class Mat // simple matrix class for homogeneous operations
{
    ...
    internal double[] Rot()
    {
        // Return the rotation part as Euler angles [rx, ry, rz] in degrees (TxyzRxyz convention)
        if (!Is4x4())
        {
            return null;
        }
        double[] txyz = this.ToTxyzRxyz();
        if (txyz == null || txyz.Length < 6)
        {
            return null;
        }
        return new double[] { txyz[3], txyz[4], txyz[5] };
    }
